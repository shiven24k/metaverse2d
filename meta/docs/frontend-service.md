...
- **Space management**: New Map modal (name/dims/template), Resize modal (5–200 in the UI), Expand modal (+10 tiles in a direction; elements, items **and NPCs** shift via `offsetX/Y`; buttons disable at the 200×200 cap), Clear All (wipes tiles, returns items to inventory), and the **Space Settings modal** (rename, 3-way privacy — Public 🌐 / Invite-only 🔑 / Private 🔒, invite-link generation/copy, member list with remove, and a **danger zone to delete the space** with two-step confirm → `DELETE /space/:id` → redirects to the lobby).
